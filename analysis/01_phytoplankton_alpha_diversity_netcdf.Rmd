...
The spdf object is stored in ../data/derived_data
...
# Use the most common name for that station cluster

station_names = spdf_data %>%
  dplyr::select(verbatimLocality,
         stationCluster) %>%
  group_by(stationCluster) %>%
  summarise(verbatimLocality = names(table(verbatimLocality))[which.max(table(verbatimLocality))])
...
# decimaLongitude, decimalLatitude, eventDate and stationCluster

dataset <- spdf_data_monthly %>%
  mutate(
    id = glue("{decimalLongitude}-{decimalLatitude}-{time}-{stationCluster}")
  )
...
station <- tibble(
  stationCluster = dataset$stationCluster,
  verbatimLocality = dataset$verbatimLocality,) %>% 
  distinct() %>%
  arrange(stationCluster)
...
longer <- expand.grid(lon = lon, lat = lat, time = time, 
                     stationCluster = station$stationCluster, 
...
longer_shannon <- longer %>% 
  mutate(
    id = glue("{lon}-{lat}-{time}-{stationCluster}")
  ) %>%
...
shannon_array <- array(
  ...
  dim = c(length(unique(spdf_data_monthly$decimalLongitude)),
          length(unique(spdf_data_monthly$decimalLatitude)),
          length(unique(spdf_data_monthly$time)),
          length(unique(spdf_data_monthly$stationCluster)))
)
...
# Define the station cluster and string80 dimensions
dim.def.nc(nc, dimname = "stationCluster", dimlength = nrow(station))
dim.def.nc(nc, dimname = "string80", dimlength = 80)

# Add station cluster variable and attributes 
var.def.nc(nc, varname = "stationCluster", vartype = "NC_INT", dimensions = "stationCluster")
att.put.nc(nc, variable = "stationCluster", name = "long_name", type = "NC_CHAR", value = "Station Cluster")

# Put station cluster data
var.put.nc(nc, variable = "stationCluster", data = station$stationCluster)

# Check
paste("station clusters")
var.get.nc(nc, variable = "stationCluster")


# Add station cluster name variable and attributes
var.def.nc(nc, varname = "verbatimLocality", vartype = "NC_CHAR", dimension = c("string80", "stationCluster"))
att.put.nc(nc, variable = "verbatimLocality", name = "standard_name", type = "NC_CHAR", value = "station_cluster_name")
att.put.nc(nc, variable = "verbatimLocality", name = "long_name", type = "NC_CHAR", value = "Station cluster name")

# Put station cluster name data
var.put.nc(nc, variable = "verbatimLocality", data = station$verbatimLocality)

# Check
paste("station cluster names")
var.get.nc(nc, variable = "verbatimLocality")
...
var.def.nc(nc, varname = "shannon", vartype = "NC_DOUBLE", dimensions = c("lon", "lat", "time", "stationCluster"))
...
var.put.nc(nc, variable = "shannon", 
           data = longer_shannon$shannon, 
           start = c(1, 1, 1, 1), 
           count = c(length(unique(spdf_data_monthly$decimalLongitude)),
                     length(unique(spdf_data_monthly$decimalLatitude)),
                     length(unique(spdf_data_monthly$time)),
                     length(unique(spdf_data_monthly$stationCluster)))) 
...
var.def.nc(nc, varname = "richness", vartype = "NC_INT", dimensions = c("lon", "lat", "time", "stationCluster"))
